...
from droid_convmamba import Droid
...
TUM_DISTORTION = {
    'fr1': np.array([0.2624, -0.9531, -0.0054, 0.0026, 1.1633]),
    'fr2': np.array([0.2312, -0.7849, -0.0033, -0.0001, 0.9172]),
    'fr3': None,  # fr3 has negligible distortion
}

def get_distortion(datapath):
    name = Path(datapath).name
    if 'freiburg1' in name:
        return TUM_DISTORTION['fr1']
    elif 'freiburg2' in name:
        return TUM_DISTORTION['fr2']
    else:
        return TUM_DISTORTION['fr3']
...
def get_intrinsics(datapath):
    ...
    if 'freiburg1' in name:
        ...
        return TUM_INTRINSICS['fr2']
    else:
        return TUM_INTRINSICS['fr3']

def image_stream(datapath):
    """ image generator """
    fx, fy, cx, cy = get_intrinsics(datapath)
    dist = get_distortion(datapath)
