"""
train_convmamba_v3.py
=====================
ConvMamba-SLAM training with single-node DDP support.

Design principles (lessons from v2):
  - OneCycleLR with FIXED --total_steps=250000 (the LR curve horizon).
    NEVER change this between resumes, or LR will collapse to 0.
  - Full state save: model + optimizer + scheduler + step + wandb_run_id.
  - Hard guards against the "LR=0 trap": validates total_steps consistency
    on resume and refuses to start if scheduler state is inconsistent.

Launch (single-node, 2 GPU H100):
  python train_convmamba_v3.py --gpus 2 --batch 2 --datapath datasets/TartanAir
  → effective batch = 4 (matches DROID-SLAM original)
"""
...
def train(gpu, args):
    ...
    if is_main:
        ...
        if wandb_run_id is not None:
            wandb.init(
                project="ConvMamba-slam", id=wandb_run_id,
                resume="must", config=vars(args),
            )
            print(f"  Resumed wandb run: {wandb_run_id}")
        else:
            wandb.init(
                project="ConvMamba-slam", name=args.name,
                config=vars(args),
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='ConvMamba_slam_v3')
    ...
    print("=" * 60)
    print(f"ConvMamba-SLAM v3 (single-node DDP, OneCycleLR)")
    print("=" * 60)
